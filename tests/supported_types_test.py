    ...
    if tp == str:
        ...
        return list(
            map(
                ...
                zip(
                    *[
                        _construct_type_examples(tp.__args__[0])
        ...
        return list(
            itertools.product(
                *tuple(_construct_type_examples(local_tp) for local_tp in tp.__args__)
            )
        )
    elif getattr(tp, "__origin__", None) in (dict, OrderedDict):
        num_vals = random.randint(15, 20)
        keys = [
            x for _ in range(num_vals) for x in _construct_type_examples(tp.__args__[0])
        ][:num_vals]
        values = [
            x for _ in range(num_vals) for x in _construct_type_examples(tp.__args__[1])
        ][:num_vals]
        ...
        values = list(
            itertools.product(
                *tuple(
                    _construct_type_examples(x.annotation)
